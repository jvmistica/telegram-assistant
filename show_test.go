...
func TestShowItem(t *testing.T) {
	...
	t.Run("one word", func(t *testing.T) {
		records := []map[string]interface{}{{"name": "egg", "amount": 12, "unit": "piece(s)", "category": "protein", "price": 98.50, "expiration": time.Date(2021, 2, 26, 20, 34, 58, 651387237, time.UTC)}}
	...
	t.Run("two words", func(t *testing.T) {
		records := []map[string]interface{}{{"name": "strawberry milk", "amount": 2, "unit": "cup(s)", "category": "fruit", "price": 98.10, "expiration": time.Date(2021, 2, 23, 20, 34, 58, 651387237, time.UTC)}}
		mocket.Catcher.Reset().NewMock().WithQuery(`SELECT * FROM "items" WHERE`).WithReply(records)
		res, err := i.ShowItem([]string{"egg"})
		assert.Nil(t, err)
